...
function validateField(field) {
  ...
  if (field === 'password') {
    errors.password = !password.value ? 'La contraseña es obligatoria' : ''
  }
}
...
function handleSocialLogin(provider) {
  Swal.fire({
    ...
    text: `La autenticación con ${provider} estará disponible próximamente.`,
    icon: 'info',
...
function handleForgotPassword() {
  Swal.fire({
    ...
    text: 'Contacta al administrador para recuperar tu contraseña.',
    icon: 'info',
    confirmButtonColor: '#2e7d32'
  })
}
...
/* Overlay degradado: transparente arriba para ver la imagen, oscuro abajo para leer el texto */
.brand-overlay {
  position: absolute;
  inset: 0;
...
/* Float label cuando tiene foco o valor */
